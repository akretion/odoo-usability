...
class StockVariationXlsx(models.TransientModel):
    ...
    categ_subtotal = fields.Boolean(
        string="Subtotals per Categories",
        default=True,
        ...
        for wiz in self:
            ...
            if not wh:
                wh = self.env["stock.warehouse"].search(
                    [("company_id", "=", wiz.company_id.id)], limit=1
                )
            if wh:
    ...
    def compute_data_from_stock(
        self, product_ids, prec_qty, start_date, end_date_type, end_date, company_id
    ):
        self.ensure_one()
        logger.debug(
            "Start compute_data_from_stock past_date=%s end_date_type=%s, end_date=%s",
            start_date,
            end_date_type,
            end_date,
        )
        ...
        ppo_loc = ppo.with_context(location=self.location_id.id).with_company(
            company_id
        )
        # Inspired by odoo/addons/stock/models/product.py
        # method _compute_quantities_dict()
        domain_quant_loc, domain_move_in_loc, domain_move_out_loc = (
            ppo_loc._get_domain_locations()
        )
        ...
        if end_date_type == "past":
            ...
            moves_in_res_end_to_present = {
                item["product_id"][0]: item["product_qty"]
                for item in smo.read_group(
                    domain_move_in_end_to_present,
            ...
            moves_out_res_end_to_present = {
                item["product_id"][0]: item["product_qty"]
                for item in smo.read_group(
                    domain_move_out_end_to_present,
                    ["product_id", "product_qty:sum"],
                    ["product_id"],
                )
            }
        ...
        moves_in_res_start_to_end = {
            item["product_id"][0]: item["product_qty"]
            for item in smo.read_group(
                domain_move_in_start_to_end,
                ["product_id", "product_qty:sum"],
                ["product_id"],
            )
        }
        moves_out_res_start_to_end = {
            item["product_id"][0]: item["product_qty"]
            for item in smo.read_group(
                domain_move_out_start_to_end,
                ["product_id", "product_qty:sum"],
                ["product_id"],
            )
        }
        ...
        for product in ppo.browse(product_ids):
            ...
            if end_date_type == "past":
                end_qty += moves_out_res_end_to_present.get(
                    product.id, 0.0
                ) - moves_in_res_end_to_present.get(product.id, 0.0)
            ...
            if (
                not float_is_zero(start_qty, precision_digits=prec_qty)
                or not float_is_zero(in_qty, precision_digits=prec_qty)
                or not float_is_zero(out_qty, precision_digits=prec_qty)
                or not float_is_zero(end_qty, precision_digits=prec_qty)
            ):
    ...
    def generate(self):
        ...
        sheet.write(
            i,
            0,
            _("Cost Price Start Date: %s") % standard_price_start_date_str,
            styles["doc_subtitle"],
        )
        i += 1
        ...
        sheet.write(
            i,
            0,
            _("Cost Price End Date: %s") % standard_price_end_date_str,
            styles["doc_subtitle"],
        )
        i += 1
        sheet.write(
            i,
            0,
            _("Stock location (children included): %s")
        ...
        sheet.write(
            i,
            0,
            _("Generated from Odoo on %s by %s") % (now_str, self.env.user.name),
        ...
        for categ_id in categ_ids:
            ...
            for l in lines:
                ...
                sheet.write_formula(
                    ...
                    cols["end_qty"]["pos"],
                    end_qty_formula,
                    styles[cols["end_qty"]["style"]],
                    l["end_qty"],
                )
                start_subtotal_formula = "=%s%d*%s%d" % (
                    letter_start_qty,
                    i + 1,
                    letter_start_price,
                    i + 1,
                ...
                sheet.write_formula(
                    i,
                    cols["start_subtotal"]["pos"],
                    start_subtotal_formula,
                    styles[cols["start_subtotal"]["style"]],
                    l["start_subtotal"],
                )
                end_subtotal_formula = "=%s%d*%s%d" % (
                    letter_end_qty,
                    i + 1,
                    letter_end_price,
                    i + 1,
                ...
                sheet.write_formula(
                    i,
                    cols["end_subtotal"]["pos"],
                    end_subtotal_formula,
                    styles[cols["end_subtotal"]["style"]],
                    l["end_subtotal"],
                )
                variation_formula = "=%s%d-%s%d" % (
                    letter_end_subtotal,
                    i + 1,
                    letter_start_subtotal,
                    i + 1,
                )
                sheet.write_formula(
                    i,
                ...
                for col_name, col in cols.items():
                    if not col.get("formula"):
                        if not l[col_name]:
                            l[col_name] = (
                                ""  # to avoid display of 31/12/1899 (dates) or '0' (char)
                            )
            ...
            if categ_subtotal:
                if categ_has_line:
                    ...
                    sheet.write_formula(
                        crow,
                        cols["categ_subtotal"]["pos"],
                        cformula,
                        styles["categ_currency"],
        ...
        # Write total
        start_total_formula = "=SUM(%s%d:%s%d)" % (
            letter_start_subtotal,
            total_row + 2,
            letter_start_subtotal,
            i + 1,
        ...
        sheet.write_formula(
            ...
            float_round(start_total, precision_rounding=prec_cur_rounding),
        )
        end_total_formula = "=SUM(%s%d:%s%d)" % (
            letter_end_subtotal,
            total_row + 2,
            letter_end_subtotal,
            i + 1,
        ...
        sheet.write_formula(
            ...
            end_total_formula,
            styles["total_currency"],
            float_round(end_total, precision_rounding=prec_cur_rounding),
        )
        variation_total_formula = "=SUM(%s%d:%s%d)" % (
            letter_variation,
            total_row + 2,
            letter_variation,
            i + 1,
        ...
        sheet.write_formula(
            total_row,
            cols["variation"]["pos"],
            variation_total_formula,
            styles["total_currency"],
            float_round(variation_total, precision_rounding=prec_cur_rounding),
        )
    ...
    def _prepare_cols(self):
        cols = {
            ...
            "start_standard_price": {
                "width": 14,
                "style": "regular_price_currency",
                "sequence": 50,
                "title": _("Start Cost Price"),
            },
            "start_subtotal": {
                "width": 16,
                "style": "regular_currency",
                "sequence": 60,
                "title": _("Start Value"),
                "formula": True,
            },
